...
export function VideoPlayer() {
  ...
  const {
    ...
    playlistLoading,
    activePlaylistId // Get active playlist ID to potentially use as album title
  } = usePlayerStore();
  ...
  const { y: scrollY } = useWindowScroll(); // Get scroll position
  ...
  useEffect(() => {
   ...
   // Effect to handle player visibility based on scroll direction
   useEffect(() => {
     ...
     const handleScroll = () => {
       ...
       // Hide if scrolling down significantly, show if scrolling up
       if (scrollDown && currentScrollY > 100) { // Hide after scrolling down 100px
         setIsVisible(false);
       } else {
         setIsVisible(true);
       }
  ...
  useEffect(() => {
      ...
      if (!currentTrack) {
          // Clear media session if no track is playing
      ...
      const playlistName = activePlaylistId ? playlists.find(p => p.id === activePlaylistId)?.name : 'VibeVerse';

      navigator.mediaSession.metadata = new MediaMetadata({
          title: currentTrack.snippet.title,
          artist: currentTrack.snippet.channelTitle,
          album: playlistName, // Use playlist name or default
          artwork: [
      ...
      // Set action handlers only if the functions exist
      if (togglePlayPause) {
      ...
      // Example for seek (if you want to implement):
      // const seekTime = 10; // seconds
      // navigator.mediaSession.setActionHandler('seekbackward', (details) => {
      //     const skipTime = details.seekOffset || seekTime;
      //     if (playerRef.current) playerRef.current.seekTo(Math.max(playerRef.current.getCurrentTime() - skipTime, 0));
      // });
      // navigator.mediaSession.setActionHandler('seekforward', (details) => {
      //      const skipTime = details.seekOffset || seekTime;
      //      if (playerRef.current) playerRef.current.seekTo(Math.min(playerRef.current.getCurrentTime() + skipTime, duration));
      // });

       // Cleanup handlers when dependencies change or component unmounts
      return () => {
  ...
  }, [isClient, currentTrack, isPlaying, playNext, playPrevious, togglePlayPause, playlists, activePlaylistId, duration]); // Add duration if using seek
  ...
  return (
    <div className={cn(
        "fixed bottom-0 left-0 right-0 h-20 md:h-24 bg-card border-t z-50 flex items-center justify-between p-2 md:p-4 gap-2 md:gap-4 transition-transform duration-300 ease-in-out",
